...
test("awaiting-login guidance follows the normalized code, never a Provider kind literal", async (t) => {
  await t.test("a browser-only synthetic runtime receives browser guidance", async (t) => {
    const clock = createClock();
    const { controller, slot } = mountConnect(t, {
      kind: "future-cli",
      service: serviceWithStatus({
        phase: "awaiting_login",
        loginUrl: "https://auth.example/browser-only",
      }),
      ...clock,
    });
    controller.start({ label: "Future" });
    button(slot, "Bắt đầu").click();
    await flush();

    const message = text(find(slot, (node) => hasClass(node, "pv-connect-message")));
    assert.match(message, /trình duyệt|browser/i);
    assert.doesNotMatch(message, /mã bên dưới/i);
    assert.equal(find(slot, (node) => hasClass(node, "pv-connect-code")), null);
  });

  await t.test("any runtime with a nonempty code receives device-code guidance", async (t) => {
    const clock = createClock();
    const { controller, slot } = mountConnect(t, {
      kind: "claude-code",
      service: serviceWithStatus({
        phase: "awaiting_login",
        loginUrl: "https://auth.example/device",
        code: "CLAUDE-DEVICE-7",
      }),
      ...clock,
    });
    controller.start({ label: "Claude" });
    button(slot, "Bắt đầu").click();
    await flush();

    const message = text(find(slot, (node) => hasClass(node, "pv-connect-message")));
    assert.match(message, /mã bên dưới/i);
    assert.equal(text(find(slot, (node) => hasClass(node, "pv-connect-code-value"))), "CLAUDE-DEVICE-7");
  });
});
