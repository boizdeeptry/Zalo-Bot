...
- [ ] **Step 1: Write failing prompt-policy tests**
  ...
  func buildAppZaloDeltaPromptResult(appZaloSessionPromptInput) appZaloDeltaPromptResult
  func appZaloShouldRotate(store.ZaloCLISession, model, fingerprint string) bool
  ```

  Assert bootstrap still comes from `buildConsultPrompt`; delta serializes the largest contiguous oldest-first prefix after cursor that fits as JSON Lines inside the fixed 16 KiB untrusted boundary, while current retrieved quotes keep their existing citable representation and file metadata stays in a separate untrusted JSONL boundary. Assert role is generated from trusted direction/operator fields rather than display name; newline and closing-tag payloads cannot escape JSON; display name, body and title are capped UTF-8-safely at 300 bytes. Reserve and pin the current event outside the prefix when necessary: suppress the synthetic question only when an inbound delta record has the same non-empty durable `ZaloMsgID` as `CurrentZaloMsgID`; same text with another ID, an empty identity, or a current ID absent from a full 100-row page must use the fallback. A pinned durable or synthetic event remains visible but cannot advance `ConsumedCursor` across a gap; the result cursor equals the last durable row in the serialized contiguous prefix and the remainder stays pending. Assert generated `CẢNH BÁO CỦA TRANG NÀY` directives retain authority in the final trust reminder and the bootstrap persona marker is absent from delta. Fingerprints must change for model, persona, roster, overlay, cite mode, `OwnerUID`, KB roots, or contract version. Rotation fires at 130,000 tokens, 48 turns, a changed fingerprint/model, or an explicit flag, but not one unit below each limit.
...
- [ ] **Step 3: Implement prompt policy**
  ...
  Hash a versioned, length-delimited sequence with SHA-256 so concatenation cannot collide, including `OwnerUID` because bootstrap embeds it in the authorization contract. Read persona, roster and thread overlay through existing helpers. Serialize transcript records with application-generated `role`, capped `display_name` and capped `body` as JSON Lines inside `<untrusted_conversation_jsonl>`; serialize capped file metadata separately inside `<untrusted_customer_files_jsonl>`. Keep retrieved KB passages in the existing citable representation. Within the 16 KiB conversation budget, serialize the largest contiguous oldest-first delta prefix and reserve room for the matching durable current event or synthetic current fallback pinned after that prefix. Return the prompt together with `ConsumedCursor`, the last durable ID in only that contiguous prefix; a pin beyond a gap never changes it, and unconsumed rows remain eligible for the next page/turn. Append the fixed trust reminder after every external-material section. The reminder must preserve the authority of application-generated page-warning directives while rejecting conversation/file data as instructions. Dedupe the current event by non-empty durable Zalo message identity only, never by text. Use byte/3 as conservative token estimate only when measured usage is unavailable.
...
- [ ] **Step 1: Write failing orchestration tests**
  ...
  With a recording command fixture, run two turns on the same thread and assert one session ID, then `--resume`; add an operator message between them and assert it is in delta. Recreate `api` with the same store and assert it still resumes. Run a second thread and assert a different UUID. Seed context at 129,999 then 130,000 tokens and assert only the latter creates a new generation. Assert 48 turns and fingerprint/model changes rotate, verified resume-not-found replaces the generation after its successful same-turn bootstrap, and `resume_unusable` marks the old mapping for a fresh bootstrap on the next turn without a second call in the failing turn. Assert the committed cursor is the content-derived high-water Claude actually received: bootstrap history maximum or resume `ConsumedCursor` from the last contiguous serialized row. Add 100 near-300-byte rows with an early operator correction and current inbound at the tail; assert the correction and pinned current are visible, cursor remains below omitted rows, and the next turn starts from the first pending row. An inbound inserted while Claude runs also remains pending for the next delta, and no separate summary/model invocation occurs.
...
- [ ] **Step 3: Implement orchestration**
  ...
  `appAnswerZalo` acquires the thread gate before starting the existing per-turn timeout, then calls `answerZalo` with a session-capable runner and advances the cursor after successful store completion. Carry the pre-run content high-water through the call: use the maximum durable ID present in bootstrap history for bootstrap/rotation/recovery, or `ConsumedCursor` returned with the bounded resume prompt. `ConsumedCursor` is the final row of the largest contiguous oldest-first prefix actually serialized from the fetched maximum-100-row page; current events pinned beyond that prefix do not advance it, and every remaining row stays pending. Never substitute the page tail or a post-run `LatestZaloMessageID`, because either can skip content Claude did not receive. Ingress is outside the gate and may also insert an unseen customer message while Claude runs. The hook extracts the current Zalo `msgId` from the last batched event's `reply.ReplyQuote` and passes that stable identity into `appRunZalo`; Task 3 does not parse reply quotes. `appRunZalo` detects that runner through a private structured-run interface; fake runners and API Provider runners continue to receive `buildConsultPrompt` through their existing `Run` method. Use compare-and-swap generation on every state update; a conflict reloads state once rather than overwriting a concurrent generation, and must fail safely without a model call if the winner still requires rotation under current policy. When the runner returns `RecoveryCode=resume_unusable` with `RecoveryAttempted=false`, mark the mapping for rotation and return the current failure without another model call; the next turn selects a new UUID and bootstrap prompt. If a verified missing-session bootstrap succeeds but persisting its effective generation fails, mark the old mapping for next-turn rotation. Estimate missing usage from the prompt of the successful attempt (bootstrap after recovery), not the failed resume delta.
...
- [ ] Prompt delta serializes the largest contiguous oldest-first prefix after the cursor within 16 KiB, pins the current event without advancing across gaps, leaves every remainder row pending, and does not repeat the bootstrap persona/contract.
